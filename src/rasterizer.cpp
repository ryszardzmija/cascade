			...
			for (int i = i_init; i <= max_i; ++i) {
				...
				if (e_i[0] <= 0 && e_i[1] <= 0 && e_i[2] <= 0) {
					...
					*uint32_ptr(char_ptr(frag_buf) + used_bytes + sizeof(uint32_t)) = j;
					...
					// Compute screen-space barycentric coordinates for the center of the fragment
					float lambda0 = e_i[1] * inv_area2;  // Coordinate for v0 computed based on the edge v1 -> v2
					float lambda1 = e_i[2] * inv_area2;  // Coordinate for v1 computed based on the edge v2 -> v0
					float lambda2 = e_i[0] * inv_area2;  // Coordinate for v2 computed based on the edge v0 -> v1

					// Precompute 1/w interpolation in screen-space for perspective-correct interpolation
					...
					*float_ptr(char_ptr(frag_buf) + used_bytes + 2 * sizeof(uint32_t)) = z_over_w_interp * inv_one_over_w_interp;

					// Compute and write perspective-correct value for the rest of the attributes
					float* attrib_ptr = float_ptr(char_ptr(frag_buf) + used_bytes + FRAGMENT_COORD_SIZE);

					for (uint32_t attrib = 0; attrib < num_attributes; ++attrib) {
						float A_over_w_interp = lambda0 * A_over_w[3 * attrib] + lambda1 * A_over_w[3 * attrib + 1] + lambda2 * A_over_w[3 * attrib + 2];
